...
class LSTMCTCModule(pl.LightningModule):
    ...
    def __init__(self, in_features, hidden_size, num_layers, dropout, bidirectional, optimizer, lr_scheduler, decoder):
        super().__init__()
        self.save_hyperparameters()
        self.model = nn.Sequential(
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            LSTMEncoder(in_features=in_features, hidden_size=hidden_size, num_layers=num_layers,
                        num_classes=charset().num_classes, dropout=dropout, bidirectional=bidirectional),
            nn.LogSoftmax(dim=-1),
        )
        self.ctc_loss = nn.CTCLoss(blank=charset().null_class)
        self.decoder = instantiate(decoder)
